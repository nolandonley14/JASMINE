...
class PPButton(ToggleButtonBehavior, Image):
    ...
    def on_state(self, widget, value):
        if value == 'down':
            self.source = 'iconFolder/png/011-pause.png'
            musicObject.player_iface.Play()
        else:
            self.source = 'iconFolder/png/012-play.png'
            musicObject.player_iface.Pause()
...
class HomeTab(TabbedPanelItem):
    ...
    def clockUpdate(self, *args):
        if len(musicObject.songTitle) > 15:
            self.title.text = musicObject.songTitle[0:15]
        else:
            self.title.text = musicObject.songTitle
        self.artist.text = musicObject.songArtist
        if musicObject.isPlaying:
            self.ppBut.source = 'iconFolder/png/011-pause.png'
        else:
            self.ppBut.source = 'iconFolder/png/012-play.png'
...
class MusicTab(TabbedPanelItem):
    title = ObjectProperty(ScrollingLabel)
    artist = ObjectProperty(Label)
    songNum = ObjectProperty(Label)
    songTime = ObjectProperty(Label)
    songDuration = ObjectProperty(Label)
